...
void cpiFire_Integrate(Box *box, Particle *particle, Thermo *thermo,
  ...
  fire->gVeloc += box_dt * fire->gForce;
  fire->sfact = exp(box_dt * fire->gVeloc);
  double phi = thermo->volFrac / pow(fire->sfact, DIM);
  if (thermo->volFrac - phi > maxDeltaVF) {
    fire->sfact =
        pow(thermo->volFrac / (thermo->volFrac - maxDeltaVF), 1.0 / DIM);
    fire->gVeloc = log(fire->sfact) / box_dt;
  } else if (phi - thermo->volFrac > maxDeltaVF) {
    fire->sfact =
        pow(thermo->volFrac / (thermo->volFrac + maxDeltaVF), 1.0 / DIM);
  ...
  thermo->volFrac /= pow(fire->sfact, DIM);
  particle->meanDiameter /= fire->sfact;
  setBasicInfo(box, particle, thermo, update, var);
...
void cbvFire_Integrate(Box *box, Particle *particle, Thermo *thermo,
                       Update *update, Variable *var) {
  minConstBoxVolFIRE *fire = update->cbvFire;
  uptriMat dt4box, sfact;
  double volScale = 1.0;
  for (int idim = 0; idim < DIM; idim++) {
    for (int jdim = idim; jdim < DIM; jdim++) {
      int sidx = spaceIdx(idim, jdim);
      if (idim == jdim) {
        double relativeP = fabs(thermo->ptensor[sidx] / thermo->pressure - 1.0);
        double dtFact =
            sqrt(particle->nAtom) * exp(-pow(relativeP / __Ptol__, 2)) + 10.0;
        dt4box[sidx] = fire->dt / dtFact;
        fire->gVeloc[sidx] += dt4box[sidx] * fire->gForce[sidx];
        sfact[sidx] = exp(dt4box[sidx] * fire->gVeloc[sidx]);
        volScale *= sfact[sidx];
      } else {
        double dtFact =
            sqrt(particle->nAtom) * exp(-pow(thermo->ptensor[sidx], 2)) + 10.0;
        dt4box[sidx] = fire->dt / dtFact;
        fire->gVeloc[sidx] += dt4box[sidx] * fire->gForce[sidx];
        sfact[sidx] = dt4box[sidx] * fire->gVeloc[sidx];
      }
    }
  }
  volScale = pow(1.0 / volScale, 1.0 / DIM);
  for (int idim = 0; idim < DIM; idim++) {
    sfact[spaceIdx(idim, idim)] *= volScale;
  }

  //=============update box=========
  for (int iedge = 0; iedge < DIM; iedge++) {
    for (int idim = 0; idim < DIM; idim++) {
      box->boxEdge[iedge][idim] *= sfact[spaceIdx(idim, idim)];
      for (int jdim = idim + 1; jdim < DIM; jdim++) {
        box->boxEdge[iedge][idim] +=
            sfact[spaceIdx(idim, jdim)] * box->boxEdge[iedge][jdim];
      }
    }
  }
  ...
  for (int idim = 0; idim < DIM; idim++) {
    ...
    thermo->volFrac = thermo->volFrac / sfact[spaceIdx(idim, idim)];
  }
  setBoxPara(box, particle, thermo, update, var);
  ...
  for (int iatom = 0; iatom < particle->nAtom; iatom++) {
    ...
    for (int idim = 0; idim < DIM; idim++) {
      pos[idim] *= sfact[spaceIdx(idim, idim)];
      for (int jdim = idim + 1; jdim < DIM; jdim++) {
        pos[idim] += sfact[spaceIdx(idim, jdim)] * pos[jdim];
      }
    }
...
void cbvFire_DotProduct(Box *box, Particle *particle, Thermo *thermo,
  ...
  for (int idim = 0; idim < DIM; idim++) {
    for (int jdim = idim; jdim < DIM; jdim++) {
      ...
      if (idim == jdim) {
        fire->gForce[sidx] =
            (thermo->ptensor[sidx] - thermo->pressure) * box->volume;
      } else {
        fire->gForce[sidx] = thermo->ptensor[sidx] * box->volume;
      }
    }
  }
...
bool cbvFire_CheckConverge(Box *box, Particle *particle, Thermo *thermo,
                           Update *update, Variable *var) {
  minConstBoxVolFIRE *fire = update->cbvFire;
  if (fire->aveForce > __AveZeroForce__) return false;
  for (int idim = 0; idim < DIM; idim++) {
    for (int jdim = idim; jdim < DIM; jdim++) {
      int sidx = spaceIdx(idim, jdim);
      if (idim == jdim) {
        double fact = fabs(thermo->ptensor[sidx] / thermo->pressure - 1.0);
        if (fact > __Ptol__) {
          return false;
        }
      } else {
        double fact = fabs(thermo->ptensor[sidx] / thermo->pressureUnits);
        if (fact > __StressTol__) {
          return false;
        }
      }
    }
...
void cptFire_Integrate(Box *box, Particle *particle, Thermo *thermo,
  ...
  for (int idim = 0; idim < DIM; idim++) {
    for (int jdim = idim; jdim < DIM; jdim++) {
      int sidx = spaceIdx(idim, jdim);
      if (idim == jdim) {
        double relativeP = fabs(
            thermo->ptensor[sidx] / fire->Pset / thermo->pressureUnits - 1.0);
        double dtFact =
            sqrt(particle->nAtom) * exp(-pow(relativeP / fire->ptol, 2)) + 10.0;
        dt4box[sidx] = fire->dt / dtFact;
        fire->gVeloc[sidx] += dt4box[sidx] * fire->gForce[sidx];
  ...
  if (fabs(phi - thermo->volFrac) > maxDeltaVF) {
    double scale = 1.0;
    if (phi > thermo->volFrac) {
      scale = pow(phi / (thermo->volFrac + maxDeltaVF), 1.0 / DIM);
    } else {
      scale = pow(phi / (thermo->volFrac - maxDeltaVF), 1.0 / DIM);
    }
    for (int idim = 0; idim < DIM; idim++) {
      sfact[spaceIdx(idim, idim)] *= scale;
      fire->gVeloc[spaceIdx(idim, idim)] =
          log(sfact[spaceIdx(idim, idim)]) / dt4box[spaceIdx(idim, idim)];
    }
  }
...
void cptFire_DotProduct(Box *box, Particle *particle, Thermo *thermo,
  ...
  for (int idim = 0; idim < DIM; idim++) {
    for (int jdim = idim; jdim < DIM; jdim++) {
      ...
      if (idim == jdim) {
        fire->gForce[sidx] =
            (thermo->ptensor[sidx] - fire->Pset * thermo->pressureUnits) *
            box->volume;
      } else {
        fire->gForce[sidx] = thermo->ptensor[sidx] * box->volume;
      }
